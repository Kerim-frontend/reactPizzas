...
function Sortpopup({ items, activeSortType, onClickSortType }) {
  ...
  const [visiblePopup, setVisiblePopup] = React.useState(false); //modal
  ...
  const dd = items.map(item =>  item.type)
  console.log(dd)
    ...
    <div ref={sortRef} className="sort">
      ...
      {visiblePopup && <div className="sort__popup">
        <ul>
          {items &&
            items.map((obj, index) => (
              <li className={activeSortType === obj.type ? 'active' : ''} onClick={() => onSelectItem(obj)} key={`${obj.name}_${index}`}>{obj.name}</li>))}
        </ul>
      </div>}
    </div>
  )
